...
VALID_GENERATION_TYPES = ["manual", "ai", "omit"]
...
def validate_resume_json(resume_json: Dict) -> Tuple[bool, List[str]]:
    ...
    if not isinstance(contact, dict):
        ...
        # Support both old format (email field) and new format (items array)
        if 'items' in contact:
            # New format with items array
            items = contact.get('items', [])
            if not isinstance(items, list):
                errors.append("contact.items must be an array")
            else:
                # Validate each item in the array
                for i, item in enumerate(items):
                    if not isinstance(item, dict):
                        errors.append(f"contact.items[{i}] must be an object")
                    else:
                        if 'icon' not in item:
                            errors.append(f"contact.items[{i}].icon is required")
                        if 'title' not in item:
                            errors.append(f"contact.items[{i}].title is required")
        else:
            # Old format with direct email field - email is optional now
    ...
    # Validate experience (supports both old flat format and new hierarchical format)
    ...
    if not isinstance(experience, list):
        ...
        for i, exp in enumerate(experience):
            if not isinstance(exp, dict):
                ...
                # Check if it's the new hierarchical format (has 'roles') or old flat format (has 'title')
                if 'roles' in exp:
                    # New hierarchical format: Company with roles
                    if 'name' not in exp or not exp['name']:
                        errors.append(f"experience[{i}].name is required")
                    if not isinstance(exp.get('roles', []), list):
                        errors.append(f"experience[{i}].roles must be an array")
                    else:
                        for j, role in enumerate(exp.get('roles', [])):
                            if not isinstance(role, dict):
                                errors.append(f"experience[{i}].roles[{j}] must be an object")
                            else:
                                if 'title' not in role or not role['title']:
                                    errors.append(f"experience[{i}].roles[{j}].title is required")
                                if not isinstance(role.get('bullets', []), list):
                                    errors.append(f"experience[{i}].roles[{j}].bullets must be an array")
                else:
                    # Old flat format
    ...
    # Validate keynotes (optional field)
    keynotes = resume_json.get('keynotes', [])
    if not isinstance(keynotes, list):
        errors.append("keynotes must be an array")
    else:
        for i, keynote in enumerate(keynotes):
            if not isinstance(keynote, dict):
                errors.append(f"keynotes[{i}] must be an object")
            else:
                if 'title' not in keynote or not keynote['title']:
                    errors.append(f"keynotes[{i}].title is required")
