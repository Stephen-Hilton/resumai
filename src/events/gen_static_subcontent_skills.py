...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    out = {"section": "skills", "mode": "static", "content": resume.get("skills") or resume.get("Skills")}
    out_path = job_path / f"subcontent.skills.yaml"
    dump_yaml(out_path, out)
    append_job_log(job_path, f"gen_static_subcontent_skills: wrote {out_path.name}")
