...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    out = {"section": "education", "mode": "static", "content": resume.get("education") or resume.get("Education")}
    out_path = job_path / f"subcontent.education.yaml"
    dump_yaml(out_path, out)
    append_job_log(job_path, f"gen_static_subcontent_education: wrote {out_path.name}")
