...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    validator = make_validators("summary", resume)

    prompt_builder = lambda inp: getattr(prompts, "summary_prompt")(inp["job"], inp["resume"], verbatim)
    fix_prompt_builder = lambda prev, errs: getattr(prompts, "summary_fix_prompt")(prev, errs)
    ...
    # Persist output
    if ok:
        dump_yaml(job_path / "subcontent.summary.yaml", payload)
        append_job_log(job_path, "gen_truthful_summary: ok")
        return EventResult(ok=True, job_path=job_path, message="ok", artifacts=["subcontent.summary.yaml"])
    else:
        append_job_log(job_path, "gen_truthful_summary: failed validation after repair")
