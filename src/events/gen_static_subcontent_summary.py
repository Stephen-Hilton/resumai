...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    out = {"section": "summary", "mode": "static", "content": resume.get("summary") or resume.get("Summary")}
    out_path = job_path / f"subcontent.summary.yaml"
    dump_yaml(out_path, out)
    append_job_log(job_path, f"gen_static_subcontent_summary: wrote {out_path.name}")
