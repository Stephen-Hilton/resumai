...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    out = {"section": "experience", "mode": "static", "content": resume.get("experience") or resume.get("Experience")}
    out_path = job_path / f"subcontent.experience.yaml"
    dump_yaml(out_path, out)
    append_job_log(job_path, f"gen_static_subcontent_experience: wrote {out_path.name}")
