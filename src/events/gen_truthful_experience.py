...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    validator = make_validators("experience", resume)

    prompt_builder = lambda inp: getattr(prompts, "experience_prompt")(inp["job"], inp["resume"], verbatim)
    fix_prompt_builder = lambda prev, errs: getattr(prompts, "experience_fix_prompt")(prev, errs)
    ...
    # Persist output
    if ok:
        dump_yaml(job_path / "subcontent.experience.yaml", payload)
        append_job_log(job_path, "gen_truthful_experience: ok")
        return EventResult(ok=True, job_path=job_path, message="ok", artifacts=["subcontent.experience.yaml"])
    else:
        append_job_log(job_path, "gen_truthful_experience: failed validation after repair")
