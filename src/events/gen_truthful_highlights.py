...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    validator = make_validators("highlights", resume)

    prompt_builder = lambda inp: getattr(prompts, "highlights_prompt")(inp["job"], inp["resume"], verbatim)
    fix_prompt_builder = lambda prev, errs: getattr(prompts, "highlights_fix_prompt")(prev, errs)
    ...
    # Persist output
    if ok:
        dump_yaml(job_path / "subcontent.highlights.yaml", payload)
        append_job_log(job_path, "gen_truthful_highlights: ok")
        return EventResult(ok=True, job_path=job_path, message="ok", artifacts=["subcontent.highlights.yaml"])
    else:
        append_job_log(job_path, "gen_truthful_highlights: failed validation after repair")
