...
async def execute(job_path: Path, ctx: EventContext) -> EventResult:
    ...
    if not html_path.exists():
        return EventResult(ok=False, job_path=job_path, message="resume.html missing")
    ...
    async with async_playwright() as p:
        ...
        await browser.close()
    append_job_log(job_path, "gen_resume_pdf: wrote resume.pdf")
    return EventResult(ok=True, job_path=job_path, message="ok", artifacts=["resume.pdf"])
